<div class="container mx-auto">
    <div class="bg-white rounded-lg shadow-md p-6" id="sale-detail">
        <!-- Header -->
        <div class="flex justify-between items-start mb-6">
            <div>
                <h1 class="text-3xl font-bold mb-2">Sale Details</h1>
                <p class="text-gray-600">Sale No: <span class="font-semibold">{{ $sale->sale_no }}</span></p>
            </div>
            <div class="flex gap-2">
                <a href="{{ route('sales.index') }}" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600 transition">
                    ← Back
                </a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 p-4 bg-gray-50 rounded-lg">
            <div>
                <h3 class="font-semibold text-gray-700 mb-3">Sale Information</h3>
                <div class="space-y-2">
                    <div class="flex justify-between">
                        <span class="text-gray-600">Sale Date:</span>
                        <span class="font-semibold">{{ $sale->sale_date->format('d M Y') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Location:</span>
                        <span class="font-semibold">{{ $sale->location->name }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-600">Customer:</span>
                        <span class="font-semibold">{{ $sale->customer_name ?? 'N/A' }}</span>
                    </div>
                </div>
            </div>
        ...
        <div class="mb-6 p-4 bg-blue-50 border-l-4 border-blue-500 rounded">
            ...
            <p class="text-gray-700">{{ $sale->remarks }}</p>
        </div>
        @endif
        ...
        <div class="p-4 bg-blue-50 rounded-lg">
            <h3 class="font-semibold text-blue-800 mb-2">✓ Stock Movement</h3>
            <p class="text-sm text-blue-700">
                This sale has been recorded in stock movements. All items have been deducted from inventory for {{ $sale->location->name }}.
            </p>
        </div>
    </div>
    <!-- Print Styles -->
